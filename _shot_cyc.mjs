...
await send('Page.navigate',{url:'http://localhost:4500/p/cyc-landscaping'});
await wait(3500);
const shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:1000,scale:2}});
const fs=await import('node:fs');
fs.writeFileSync('/tmp/cyc_hero.png',Buffer.from(shot.result.data,'base64'));
console.log('done');
