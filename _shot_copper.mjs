...
await send('Page.navigate',{url:'http://localhost:4500/p/copper-valley-landscaping'});
await wait(4000);
const fs=await import('node:fs');
// hero (top viewport)
let shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:900,scale:1.5}});
fs.writeFileSync('/tmp/copper_hero.png',Buffer.from(shot.result.data,'base64'));
// services grid
const ev=await send('Runtime.evaluate',{expression:`(()=>{const a=[...document.querySelectorAll('a[href*="/services/"]')].find(e=>e.querySelector('img')); if(!a) return JSON.stringify({y:1100,h:1300}); const r=a.getBoundingClientRect(); return JSON.stringify({y:Math.max(0,r.top+window.scrollY-120), h:1250});})()`,returnByValue:true});
...
fs.writeFileSync('/tmp/copper_svc.png',Buffer.from(shot.result.data,'base64'));
