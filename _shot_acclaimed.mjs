...
const UD='/Users/lauren/Documents/Lauren\'s Repo/buildlocal/.tmp/chrome-shot';
const p=spawn(CHROME,['--headless=new',`--remote-debugging-port=${port}`,`--user-data-dir=${UD}`,'--disk-cache-dir='+UD+'/cache','--no-first-run','--hide-scrollbars','--window-size=1440,1000','about:blank'],{stdio:'ignore'});
await wait(2000);
...
await send('Page.navigate',{url:'http://localhost:4500/p/acclaimed-roofing-scottsdale-residential-and-commercial-roof'});
await wait(4000);
const fs=await import('node:fs');
// hero
let shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:1000,scale:1.5}});
fs.writeFileSync('/tmp/acc_hero.png',Buffer.from(shot.result.data,'base64'));
// services grid
...
fs.writeFileSync('/tmp/acc_svc.png',Buffer.from(shot.result.data,'base64'));
