...
await send('Page.navigate',{url:'http://localhost:4500/p/monsoon-roofing-inc'});
await wait(9000);
...
for(let n=0;n<bands.length;n++){
  ...
  if(shot?.result?.data) fs.writeFileSync(`_shots/monsoon-${n}.png`,Buffer.from(shot.result.data,'base64'));
}
console.log('done');
