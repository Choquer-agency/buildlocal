...
await send('Page.navigate',{url:'http://localhost:4500/p/real-roofing'});
await wait(4500);
let shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:1000,scale:1.5}});
fs.writeFileSync('_shots/real_hero.png',Buffer.from(shot.result.data,'base64'));
...
fs.writeFileSync('_shots/real_svc.png',Buffer.from(shot.result.data,'base64'));
console.log('ok y',y);
