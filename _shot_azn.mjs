...
await send('Page.navigate',{url:'http://localhost:4500/p/aznativescapes'});
await wait(3800);
// hero + nav
let shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:900,scale:1.5}});
fs.writeFileSync('/tmp/azn_hero.png',Buffer.from(shot.result.data,'base64'));
// services grid
const ev=await send('Runtime.evaluate',{expression:`(()=>{const a=[...document.querySelectorAll('a[href*="/services/"]')].find(e=>e.querySelector('img'))||[...document.querySelectorAll('a[href*="/services/"]')][0]; if(!a) return JSON.stringify({y:1100,h:1200}); const r=a.getBoundingClientRect(); return JSON.stringify({y:Math.max(0,r.top+window.scrollY-110), h:1000});})()`,returnByValue:true});
...
fs.writeFileSync('/tmp/azn_svc.png',Buffer.from(shot.result.data,'base64'));
console.log('ok grid y',y);
