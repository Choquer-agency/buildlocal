...
await send('Page.navigate',{url:'http://localhost:4500/p/falcon-landscapes-pavers-and-masonry-llc'});
await wait(4000);
const fs=await import('node:fs');
// hero
let shot=await send('Page.captureScreenshot',{format:'png',clip:{x:0,y:0,width:1440,height:1000,scale:1.5}});
fs.writeFileSync('/tmp/falcon_hero.png',Buffer.from(shot.result.data,'base64'));
// services + why-us region
shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:true,clip:{x:0,y:1000,width:1440,height:1500,scale:1.5}});
fs.writeFileSync('/tmp/falcon_mid.png',Buffer.from(shot.result.data,'base64'));
console.log('done');
